...
/**
 * table for song and corresponding lyrics data also auxiliary pointers to fused data index
 *
 */
public class LyricsTable {
    ...
    /*public synchronized String getTransferableData(BigInteger hashValue) {
    	List<String> transferableData = new ArrayList<String>();
    	for(String songName : listOfLyricsData.keySet()) {
    		int compare = HashUtils.getHash(songName).compareTo(hashValue);
    		if(compare == -1 || compare == 0) {
    			transferableData.add(songName+MessageConstants.SPACE+listOfLyricsData.get(songName));
    			listOfLyricsData.remove(songName);
    		}
    	}
    	return MessageConstants.constructMessage((String[])transferableData.toArray());
    }*/
